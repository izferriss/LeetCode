...
class DetectSquares
{
    ...
    count(point, {points} = this, score = 0)
    {
        ...
        {
            let isSame = (Math.abs(x2 - x1) == Math.abs(y2 - y1));
            let isEqual = ((x1 == x2) || (y1 == y2));
            let canSkip = (!isSame || isEqual);
            if(canSkip){continue;}
    ...
    {
        let [aK, bK] = [this.getKey(x1, y2), this.getKey(x2, y1)];
        let [aS, bS] = [(map[aK] || 0), (map[bK] || 0)];
        return (aS * bS);
    }
}